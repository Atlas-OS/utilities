...
fn main() {
    ...
    let hive = winreg::RegKey::load_app_key(file.expect("Failed to load hive!"), true);
    let components = hive
        .unwrap()
        .open_subkey("DerivedData\\Components")
        .expect("Could not locate DerivedData\\Components subkey! Are you sure you selected the right hive?");
    ...
    let scan = [
        ...
        "microsoft-windows-storagemigration",       // stormig.dll
        "xtservice-migration",
        "upgrade.resources",
    ...
    for component in components_vec {
        ...
        if re.is_match(component_name) {
            ...
            components.delete_subkey(component_name).expect("Could not delete subkey!");
        }
    }
}
